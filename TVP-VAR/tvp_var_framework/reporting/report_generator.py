...
class ReportGenerator:
    ...
    def add_stationarity_report(self, stationarity_results, warnings=None):
        """添加平稳性检验章节"""
    ...
    def _render_stationarity(self, results):
        ...
        for name, result in results.items():
            if isinstance(result, dict) and "statistic" in result:
                ...
                if "adf_is_stationary" in result and "kpss_is_stationary" in result:
                    status = f"{status} (ADF={'是' if result['adf_is_stationary'] else '否'}, KPSS={'是' if result['kpss_is_stationary'] else '否'})"
    ...
    def _render_forecast(self, section):
        ...
        for s in range(min(len(labels), pred_mean.shape[0])):
            row = f"| {labels[s]} |"
            for i in range(len(var_names)):
                row += f" {pred_mean[s, i]:.4f}[{pred_lower[s, i]:.4f},{pred_upper[s, i]:.4f}] |"
            lines.append(row)
        return "\n".join(lines)
