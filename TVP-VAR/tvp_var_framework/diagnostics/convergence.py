...
def parameter_is_converged(param_diag, rhat_threshold=1.1, ess_min=100, geweke_p_min=0.05):
    """Use one consistent rule for per-parameter and aggregate convergence."""
    rhat = param_diag.get("rhat", np.inf)
    ess = param_diag.get("ess", 0)
    geweke_p = param_diag.get("geweke_p", np.nan)

    rhat_ok = np.isfinite(rhat) and rhat <= rhat_threshold
    ess_ok = np.isfinite(ess) and ess >= ess_min
    geweke_ok = np.isnan(geweke_p) or geweke_p >= geweke_p_min
    return rhat_ok and ess_ok and geweke_ok
...
class ConvergenceReport:
    ...
    def is_converged(self, rhat_threshold=1.1, ess_min=100, geweke_p_min=0.05):
        for model_name, diag in self.diagnostics.items():
            ...
            for param_name, param_diag in diag.items():
                if isinstance(param_diag, dict):
                    param_geweke_p_min = (
                        0.0
                        if param_diag.get("diagnostic_note") == "analytic_iid"
                        else geweke_p_min
                    )
                    if not parameter_is_converged(
                        param_diag,
                        rhat_threshold=rhat_threshold,
                        ess_min=ess_min,
                        geweke_p_min=param_geweke_p_min,
                    ):
                        return False
        return True
