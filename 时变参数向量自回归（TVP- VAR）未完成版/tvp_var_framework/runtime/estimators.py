...
def _build_joint_chain_from_model(model, model_type):
    """
...
def estimate_bayesian(inputs: dict, params: dict) -> dict:
    ...
    for q in [1e-7, 1e-5, 1e-3, 1e-1]:
        comparison.add_model(f"q={q:.0e}", BayesianTVP_VAR(n_vars=n, q=q, r=1e-3))
    ...
    means, lowers, uppers = bma.predict_bma_interval(Y, steps=fc_steps, n_samples=fc_samples)
    if normalize:
    ...
    for sv_idx, sn in enumerate(names):
        best_model.impulse_response(shock_var=sv_idx, shock_size=1.0, periods=irf_periods, n_samples=300)
    ...
    joint_chain = _build_joint_chain_from_model(best_model, "bayesian")
...
def estimate_v2(inputs: dict, params: dict) -> dict:
    ...
    preds = mc_pred.predict(
        ...
        Y[-1], steps=4, n_samples=min(300, fc_samples),
    )
