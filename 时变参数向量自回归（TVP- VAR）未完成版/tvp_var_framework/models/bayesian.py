...
class BayesianTVP_VAR:
    ...
    def __init__(self, n_vars=3, q=1e-5, r=1e-3, prior_mean=None, prior_cov=None):
        """
        参数:
            n_vars: 变量数
            q: 过程噪声方差 (状态转移噪声)
        ...
        self.q = q
        self.r = r
        self.analyst = TVP_VAR_Analyst(n_vars=n_vars, q=q, r=r)
    ...
    def fit(self, Y):
        ...
        """
        self.log_likelihoods = []
        self.analyst = TVP_VAR_Analyst(n_vars=self.n_vars, q=self.q, r=self.r)

        for t in range(1, len(Y)):
            y_prev = Y[t - 1]
            y_true = Y[t]

            Z = self.analyst._build_Z(y_prev)
            ...
            # Kalman 更新
            self.analyst.update(y_prev, y_true)
    ...
    def sample_trajectory(self, n_samples=100):
        ...
        for record in self.analyst.history:
            c = record["c"]
            A = record["A"]
            theta = np.concatenate([c, A.flatten()])
            # 使用最终的 P 作为近似 (简化)
    ...
    def predict_interval(self, Y, steps=1, n_samples=1000):
        ...
        """
        preds = self.predict_samples(Y, steps, n_samples)
...
class BayesianModelAveraging:
    ...
    def predict_bma(self, Y, steps=1, n_samples=500):
        ...
        for name, model in self.comparison.models.items():
            w = self.weights[name]
            means, _, _ = model.predict_interval(Y, steps=steps, n_samples=n_samples)
    ...
    def predict_bma_interval(self, Y, steps=1, n_samples=500):
        ...
        for name, count in zip(model_names, counts):
            ...
            # 从每个模型的后验预测分布中采样
            preds = model.predict_samples(Y, steps=steps, n_samples=count)
